...
def solve_vrp(nodes_df, requests_df, vehicles_df, time_matrix, batch_size):
    """
    ...
    # ============================================================
    # 1A. BATCHING I POJEMNOŚĆ
    # ============================================================



    # liczba batchy w labie
    num_samples = dict(zip(
        requests_small["lab_node_id"],
        requests_small["num_samples"]
    ))

    demand_batches = {
        i: int(np.ceil(num_samples[i] / batch_size))
        for i in lab_nodes
    }

    # UWAGA: capacity w vehicles_df oznacza liczbę batchy
    capacity_batches = dict(zip(
        vehicles_small["vehicle_id"],
        vehicles_small["capacity"]
    ))
    ...
    ######3 batching
        # ładunek pojazdu k po obsłużeniu węzła i (w paczkach)
    load = LpVariable.dicts(
        "load",
        ((k, i) for k in vehicles for i in all_nodes),
        lowBound=0
    )
    ...
    for k in vehicles:
        for i in all_nodes:
            for j in lab_nodes:   # tylko laby mają demand
                if (i, j) not in travel_cost:
                    continue

                demand_j = demand_batches[j]

                model += (
                    load[(k, i)] + demand_j
                    <= capacity_batches[k] + M * (1 - x[(k, i, j)])
                )
    ...
    # ============================================================
    # 16B. ŁADUNEK TYLKO DLA FAKTYCZNEJ TRASY
    # ============================================================
    ...
    for k in vehicles:
        # trasa pojazdu k
        route_k = routes_df[routes_df["vehicle_id"] == k].sort_values("stop_order")

        for _, row in route_k.iterrows():
            node = row["node"]
            route_load_rows.append({
                "vehicle_id": k,
                "stop_order": row["stop_order"],
                "node": node,
                "load_after_node": value(load[(k, node)])
            })

    route_load_df = pd.DataFrame(route_load_rows)
    ...
    # ============================================================
    # 18D. ŁADUNEK POJAZDÓW NA TRASIE
    # ============================================================
    ...
    for k in vehicles:
        for i in all_nodes:
            load_rows.append({
                "vehicle_id": k,
                "node": i,
                "load_after_node": value(load[(k, i)])
            })
    ...
    return {
        ...
        "route_load": route_load_df,
        "travel_time_summary": travel_time_summary,
        "total_route_time": total_route_time_df,
        "load": load_df
    }
